...
use super::types::{FileSelector, SelectorEntry, SelectorEntryKind};
use crate::client_lib::util::config::FILES_FOR_TRANSFER;
use anyhow::Result;
...
impl FileSelector {
    ...
    pub fn update_entries(&mut self) -> Result<()> {
        ...
        self.entries[self.selected_index].selected = true;
        Ok(())
    }
}
